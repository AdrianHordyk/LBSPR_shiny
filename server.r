...
shinyServer(function(input, output, clientData, session) {
  ...
  ExampleDataFile <- reactive({
    switch(input$exampData,
	  rawSingHead = "data/LRaw_SingYrHead.csv",
	  rawSing = "data/LRaw_SingYr.csv",
	  rawMultiHead = "data/LRaw_MultiYrHead.csv",
	  rawMulti = "data/LRaw_MultiYr.csv",
	  freqSingHead = "data/LFreq_SingYrHead.csv",
	  freqSing = "data/LFreq_SingYr.csv",
	  freqMultiHead = "data/LFreq_MultiYrHead.csv",
	  freqMulti = "data/LFreq_MultiYr.csv")
  })
  ...
  getLB_lens <- reactive({
   ...
   if (!chkSep()) return(NULL)
     ...
     LB_lengths <- new("LB_lengths", file=dat, LB_pars=LB_pars, dataType=input$dataType)
	 LB_lengths
  })
  ...
  # addTooltip(session, id="header", title = "Does the first row contain labels?",
           # placement = "right", trigger = "hover")
  # # addTooltip(session, id = "beta", title = "",
          # # placement = "right", trigger = "hover")
  ...
  # observe({
	...
	# }
   	...
   	# if (L95 <= L50) {
      # createAlert(session,  "lmalert", "lmpar", title = "Error",
        # content = "L50 must be less than L95", append = FALSE)
    # } else {
	  # closeAlert(session, "lmpar")
	# }
	...
	# # Warnings
	# if (L50 >= Linf | L95 >= Linf) {
      # createAlert(session,  "lmalert2", "lmpar2", title = "Warning",
        # content = "Are you sure maturity parameters are higher than Linf?",
		# append = FALSE)
    # } else {
	  # closeAlert(session, "lmpar2")
	# }
	# if (Linf < 10) {
      # createAlert(session,  "linfalert", "linf1", title = "Warning",
        # content = "Are you sure Linf is so low?",
		# append = FALSE)
    # } else {
	  # closeAlert(session, "linf1")
	# }
	# if (MK < 0.2 | MK > 6) {
      # createAlert(session,  "mkalert", "mk1", title = "Warning",
        # content = "Are you sure of the M/K value? Model may not perform well
		  # at extreme values",
		# append = FALSE)
    # } else {
	  # closeAlert(session, "mk1")
  ...
  # ChckPars <- reactive({
	...
	# if (is.na(L50)) L50 <- 0
	# if (is.na(L95)) L95 <- 0
	# if (any(c(MK, Linf, L50, L95) <= 0)) error <- TRUE
	# error
  # })



  # # Read CSV file, print first 6 rows, and create histogram of length data




  # output$hist <- renderPlot({
    # # if(is.null(chkDat())) return(NULL)
	# # if(!chkNumeric()) return(NULL)
	# # # if(!chckEqDst()) return(NULL)
	# print(plotSize(getLB_lens()))
  # })
  ...
  # chckEqDst <- reactive({
	...
	# # type <- indat$type
	# # if (type=="FREQ") {
	  # # if (all(diff(dat[,1]) != diff(dat[,1])[1])) return(FALSE)
	# # }
	# return(TRUE)
  # })
  # chkNumeric <- reactive({
	...
	# dat <- indat
	# dat <- as.matrix(dat)
	# if (all(apply(dat, 2, is.numeric))) return(TRUE)
	# if (!all(apply(dat, 2, is.numeric))) return(FALSE)
  # })
  ...
  # output$chkDatLoad <- renderText ({
    # if(is.null(chkDat())) return("No data file has been uploaded")
	# if(ChckPars()) return("Input parameters missing")
  # })

  # values <- reactiveValues(last=0, ShowLine=TRUE, fm=NULL, wfm=NULL, sl50=NULL,
    # sl95=NULL,spr=NULL, predlen=NULL)
  # observeEvent(input$goButton, {
    # if(input$goButton == 0) return(NULL)
    # values$last <- input$goButton - 1
	# values$ShowLine <- TRUE
  # })

  # observeEvent(input$file1, {
	# values$ShowLine <- FALSE
  # })
  # observeEvent(input$binswidth, {
    # values$ShowLine <- FALSE # Add update if other parameters change
  # })
  # # If any biological parameters are changed, the assessment is reset
  # observeEvent(input$MK, {
    # values$ShowLine <- FALSE # Add update if other parameters change
  ...
  # doAssess <- reactive({
    # # if(is.null(chkDat())) return(NULL)
	# # if(!chkNumeric()) return(NULL)
	# # if(ChckPars()) return(NULL)

	# LB_fit <- LBSPRfit(getLB_pars(), getLB_lens())
  # })
  # # output$debug <- renderText({values$ShowLine})

  # # Histogram of Length Data
  # output$HistPlot <- renderPlot({
    # if (values$ShowLine != TRUE)  print(plotSize(getLB_lens()))
    # if (values$ShowLine == TRUE) print(plotSize(doAssess()))
  # })
  # addPopover(session, "HistPlot", "Length Distribution",
    # content = paste0("Model fit: solid black line (if model run)"),
    # trigger="hover")

  # output$MatSel <- renderPlot({
    # if (values$ShowLine != TRUE)  print(plotMat(getLB_pars()))
    # if (values$ShowLine == TRUE) print(plotMat(doAssess()))
  # })
  # addPopover(session, "MatSel", "Maturity & Selectivity at Length",
    # content = paste0("Maturity-at-Length: solid line", tags$br(),
	                 # "Selectivity-at-Length: dashed line (if model run)"),
    # trigger="hover")

  # output$SPRCirc <- renderPlot({
    # if (values$ShowLine != TRUE) return(NULL)
	# if (values$ShowLine == TRUE) plotSPRCirc(doAssess(), SPRTarg=input$sprtarg, SPRLim=input$sprlim)
  # })
  # observe({
    # if(values$ShowLine) {
    # addPopover(session, "SPRCirc", "Estimated SPR",
      # content = paste0("Green = Above SPR Target", tags$br(), "
	          # Orange = Between SPR Target and Limit", tags$br(),
			  # "Red = Below SPR Limit"),
      # trigger="hover")
    # }
  # })
  ...
  # makeDF <- function() {
	...
	# if(ChckPars()) return(NULL)
    # if(values$ShowLine) {
      # DF <- data.frame(SPR=values$spr, SL50=values$sl50, SL95=values$sl95,
	  # FM=values$fm, wFM=values$wfm)
	  # DF <- round(DF, 2)
	  # colnames(DF) <- c(
	    # 'SPR',
		# '<em>SL<sub>50</sub></em>',
  ...
  # makeDF2 <- function() {
    ...
    # if(values$ShowLine) {
	  ...
	  # FM=values$fm, wFM=values$wfm)
	  # DF <- round(DF, 2)
	  # colnames(DF) <- c("SPR", "SL50", "SL95", "F/M", "wF/M")
	# DF
  # }}
  # # Table of Results
   # output$results <- renderDataTable({
     # makeDF()
   # }, escape = FALSE, options=list(pageLength=-1, searching = FALSE, paging = FALSE,
     # ordering=FALSE, info=FALSE, rowCallback = I(
    # 'function(row, data) {
        # $("td", row).css("text-align", "center");
      # }'
  ...
  # output$downloadPlot <- downloadHandler(
    ...
    # content = function(file) {
	  ...
	  # layout(matrix(c(1, 2, 3, 4,4,4), nrow = 3, ncol = 2, byrow = TRUE))
	  # # par(mfrow=c(2,2))
      ...
      # grob <-  tableGrob(makeDF2(), theme=ttheme_minimal())
      # grid.draw(grob)
      # popViewport(3)
      # dev.off()
    # })
